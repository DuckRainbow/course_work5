...
def main():
    ...
    while True:
        ...
        if int(user_answer) == 1:
            data = dbmanager.get_companies_and_vacancies_count(params)
            print(data)
        elif int(user_answer) == 2:
            data = dbmanager.get_all_vacancies(params)
            print(data)
        elif int(user_answer) == 3:
            data = dbmanager.get_avg_salary(params)
            print(data)
        elif int(user_answer) == 4:
            data = dbmanager.get_vacancies_with_higher_salary(params)
            print(data)
        elif int(user_answer) == 5:
            user_word = input('Введите слово, по которому будем искать вакансии: ')
            data = dbmanager.get_vacancies_with_keyword(params, user_word)
            print(data)
        else:
            break
